...
| 파일 | 변경 유형 |
|---|---|
| [`domain/services.py`](backend/app/domain/services.py) | `generate_suggested_questions` 추상 메서드 추가 |
| [`infrastructure/external/gemini_service.py`](backend/app/infrastructure/external/gemini_service.py) | `GeminiLLMService` & `SimulatedGeminiService` 구현 추가 |
| [`usecases/generate_usecase.py`](backend/app/usecases/generate_usecase.py) | Step 7 `done` 이벤트에 `suggested_questions` 포함 |

### 프론트엔드

| 파일 | 변경 유형 |
|---|---|
| [`entities/message/model/types.ts`](frontend/src/entities/message/model/types.ts) | `Message` 타입에 `suggested_questions?: string[]` 추가 |
| [`shared/api/sseClient.ts`](frontend/src/shared/api/sseClient.ts) | `SSEChunk` 타입 확장 |
| [`features/send-message/api/sendMessage.ts`](frontend/src/features/send-message/api/sendMessage.ts) | `onSuggestedQuestions` 콜백 처리 추가 |
| [`entities/message/ui/ChatMessage.tsx`](frontend/src/entities/message/ui/ChatMessage.tsx) | 추천 질문 버튼 렌더링 추가 |
| [`entities/message/ui/ChatMessage.module.css`](frontend/src/entities/message/ui/ChatMessage.module.css) | `.suggestedQuestions`, `.suggestedBtn` 스타일 추가 |
| [`widgets/chat-feed/ui/ChatFeed.tsx`](frontend/src/widgets/chat-feed/ui/ChatFeed.tsx) | `onSuggestedQuestions` 연결, `handleFollowUpQuestion` 구현 |
